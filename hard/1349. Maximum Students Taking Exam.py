...
class Solution:
    def maxStudents(self, seats: List[List[str]]) -> int:
        ...
        1. (mask & (mask' >> 1)) == 0, there should be no students in the
            upper left position for every student.
        2. (mask & (mask' << 1)) == 0, there should be no students in the
            upper right position for every student.
        '''
        R, C = len(seats), len(seats[0])
        ...
        bitCounts = [bin(mask).count("1") for mask in range(1 << C)]
