...
class Skiplist:
    ...
    The critical method to implement is _skip_search(), where we start from the
    top of the skiplist, and return the largest node whose value is less than
    or equal to the target.
    Based on _skip_search(), we can find the insertion place to add the new
    number, when adding, use a coin-flip like process to determine whether we
    want to add the node to the upper level as well.
    Based on _skip_search(), we can find the node to delete, and we delete all
    ...
    def search(self, target: int) -> bool:
        p = self._skip_search(target)

        return p.val == target

    def add(self, num: int) -> None:
        prev = self._skip_search(num)
        down = None
        i = 0
        while True:
            cur = LinkedNode(num)
